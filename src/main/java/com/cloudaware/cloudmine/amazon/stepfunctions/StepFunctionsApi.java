...
public final class StepFunctionsApi {
    ...
    @ApiMethod(
            httpMethod = ApiMethod.HttpMethod.GET,
            name = "stateMachines.executions.get",
            path = "{region}/state-machines/{stateMachineArn}/executions/{executionArn}"
    )
    public ExecutionResponse stateMachinesExecutionsGet(
            @Named("credentials") final String credentials,
            @Named("region") final String region,
    ...
    @ApiMethod(
            httpMethod = ApiMethod.HttpMethod.GET,
            name = "activities.activityTask.get",
            path = "{region}/activities/{activityArn}/activity-task"
    )
    public ActivityTaskResponse activitiesActivityTaskGet(
            @Named("credentials") final String credentials,
            @Named("region") final String region,
            @Named("activityArn") final String activityArn
    ) throws AmazonUnparsedException {
        return StepFunctionsCaller.get(GetActivityTaskRequest.class, ActivityTaskResponse.class, credentials, region).execute((client, request, response) -> {
            final GetActivityTaskResult result = client.getActivityTask(request.withActivityArn(activityArn));
            response.setInput(result.getInput());
            response.setTaskToken(result.getTaskToken());
        });
    }

    @ApiMethod(
            httpMethod = ApiMethod.HttpMethod.GET,
            name = "stateMachines.executions.executionHistory.list",
            path = "{region}/state-machines/{stateMachineArn}/executions/{executionArn}/execution-history"
    )
    public ExecutionHistoryResponse stateMachinesExecutionsExecutionHistoryList(
            @Named("credentials") final String credentials,
            @Named("region") final String region,
            @Named("executionArn") final String executionArn,
            @Named("page") @Nullable final String page
    ) throws AmazonUnparsedException {
        return StepFunctionsCaller.get(GetExecutionHistoryRequest.class, ExecutionHistoryResponse.class, credentials, region).execute((client, request, response) -> {
            final GetExecutionHistoryResult result = client.getExecutionHistory(
                    request
                            .withExecutionArn(executionArn)
                            .withNextToken(page)
            );
            response.setHistoryEvents(result.getEvents());
            response.setNextPage(result.getNextToken());
        });
    }
}
